...
def copy_to_clipboard(text: str) -> bool:
    ...
    # Windows: prefer the built-in `clip` utility (no GUI required).
    if sys.platform == "win32":
        try:
        ...
        # Fallback: PowerShell clipboard API
        try:
            subprocess.run(
    ...
    # Cross-platform fallback (requires a GUI-capable environment).
    try:
        r = tk.Tk()
        r.withdraw()
        r.clipboard_clear()
        r.clipboard_append(text)
        r.update()  # keeps the clipboard contents after the window is closed
        r.destroy()
        return True
    except Exception:
        return False
            ...
            for index, task in enumerate(asyncio.as_completed(task_objects), 1):
                try:
                    ...
                    if url:
                        ...
                        if copy_to_clipboard(successful_url):
                            print("→ Link was copied to clipboard.\n")
